CREATE TABLE `customer_staff`(
    ...
    `system_id`       bigint(20) DEFAULT NULL COMMENT '外部系统ID(租户ID),非空则表示该客服人员数据来自外部系统',
...
CREATE TABLE `outsourcing_system`(
    ...
    `system_url`  varchar(100) NOT NULL COMMENT '系统访问URL,用于数据拉取同步',
    ...
    `update_time` datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='客服外部系统(租户)'